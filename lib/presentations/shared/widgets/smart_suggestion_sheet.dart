  ...
  static List<ChecklistSuggestion> analyze({
    ...
    if (weather != null) {
      ...
      if (desc.contains('mưa') ||
        ...
        raw.addAll([
          const ChecklistSuggestion(
              emoji: '☂️', title: 'Ô / áo mưa', categoryId: 'cat_4'),
          const ChecklistSuggestion(
              emoji: '👟', title: 'Giày chống nước', categoryId: 'cat_4'),
        ]);
      }
      if (desc.contains('sương') || temp < 18) {
        raw.addAll([
          const ChecklistSuggestion(
              emoji: '🧥', title: 'Áo khoác ấm', categoryId: 'cat_4'),
          const ChecklistSuggestion(
              emoji: '🧣', title: 'Khăn quàng cổ', categoryId: 'cat_4'),
        ]);
      }
      // Chỉ gợi ý đồ nắng khi trời thật sự nắng (không mưa, không dông)
      final isRainy = desc.contains('mưa') ||
          desc.contains('dông') ||
          desc.contains('phùn') ||
          desc.contains('sương');
      if (!isRainy &&
        ...
        raw.addAll([
          ...
          const ChecklistSuggestion(
              ...
              categoryId: 'cat_4'),
          const ChecklistSuggestion(
              emoji: '🎩', title: 'Mũ / nón rộng vành', categoryId: 'cat_4'),
        ]);
      }
    }
    ...
    if (_has(all, ['chùa', 'đền', 'phủ', 'miếu', 'lễ', 'thờ', 'lăng'])) {
      raw.addAll([
        const ChecklistSuggestion(
            emoji: '💵', title: 'Tiền lẻ công đức', categoryId: 'cat_1'),
        const ChecklistSuggestion(
            emoji: '📜', title: 'Sớ / vàng mã', categoryId: 'cat_1'),
        const ChecklistSuggestion(
            emoji: '👘',
            title: 'Trang phục kín đáo',
            categoryId: 'cat_4'),
        const ChecklistSuggestion(
            emoji: '🏮', title: 'Hoa tươi cúng lễ', categoryId: 'cat_1'),
      ]);
    }
    ...
    if (_has(all, [
      ...
      raw.addAll([
        const ChecklistSuggestion(
            emoji: '⛽', title: 'Đổ đầy xăng xe', categoryId: 'cat_4'),
        const ChecklistSuggestion(
            emoji: '🗺️', title: 'Tải bản đồ offline', categoryId: 'cat_4'),
        const ChecklistSuggestion(
            emoji: '🍱',
            title: 'Đồ ăn nhẹ đường dài',
            categoryId: 'cat_3'),
        const ChecklistSuggestion(
            emoji: '💧', title: 'Nước uống (1.5L)', categoryId: 'cat_3'),
        const ChecklistSuggestion(
            emoji: '🏥', title: 'Thuốc say xe', categoryId: 'cat_4'),
      ]);
    }
    ...
    if (_has(all, [
      ...
      raw.addAll([
        const ChecklistSuggestion(
            emoji: '🛍️', title: 'Túi tote đựng đồ', categoryId: 'cat_1'),
        const ChecklistSuggestion(
            emoji: '📦',
            title: 'Hộp / giấy gói quà',
            categoryId: 'cat_1'),
        const ChecklistSuggestion(
            emoji: '💰', title: 'Tiền mặt tiêu vặt', categoryId: 'cat_1'),
      ]);
    }
    ...
    if (_has(all, [
      ...
      raw.addAll([
        const ChecklistSuggestion(
            emoji: '🔋', title: 'Pin dự phòng', categoryId: 'cat_4'),
        const ChecklistSuggestion(
            emoji: '🤳',
    ...
    if (_has(all, ['hồ', 'công viên', 'đạp xe', 'đi bộ', 'đi dạo'])) {
      raw.addAll([
        const ChecklistSuggestion(
            ...
            categoryId: 'cat_4'),
        const ChecklistSuggestion(
            emoji: '🧢', title: 'Mũ lưỡi trai', categoryId: 'cat_4'),
      ]);
    }
    ...
    raw.addAll([
      const ChecklistSuggestion(
          emoji: '🪪', title: 'CMND / CCCD', categoryId: 'cat_4'),
      const ChecklistSuggestion(
          emoji: '🔑', title: 'Chìa khoá nhà / xe', categoryId: 'cat_4'),
      const ChecklistSuggestion(
          emoji: '🩹',
          title: 'Băng dán / thuốc cơ bản',
          categoryId: 'cat_4'),
      const ChecklistSuggestion(
          emoji: '📱', title: 'Điện thoại sạc đầy', categoryId: 'cat_4'),
      // Quan trọng nhất — luôn mang theo 😄
      const ChecklistSuggestion(
          emoji: '😄', title: 'Một tâm hồn đẹp', categoryId: 'cat_4'),
    ]);
              ...
              Expanded(
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Dựa trên lịch trình & thời tiết',
                        style: GoogleFonts.plusJakartaSans(
              ...
              GestureDetector(
                onTap: () => setState(() {
                  allSelected
                      ? _selected.clear()
                      : _selected
                          .addAll(List.generate(_suggestions.length, (i) => i));
                }),
                child: Container(
                  padding:
